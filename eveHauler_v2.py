...
class Hauler:
    ...
    def get_new_mission(self, max_route_length):
        ...
        while True:
            ...
            if self.route_waypoint_count() != 0:
                ...
                left_click(self.ui_tree.find_node({'_name': 'context_menu_Set Destination'}))
                right_click(route_markers[0].get_center())
                left_click(self.ui_tree.find_node({'_name': 'context_menu_Remove Waypoint'}))
            right_click(self.ui_tree.find_node({'_name': 'tablecell 1-3'}).get_center())
            time.sleep(1)
            ...
            if route_length <= max_route_length:
                ...
                btn_decline = btn_group.find_image('images/btn_decline.png', 10, 0.7)
                ...
                while not self.can_accept():
                    dialog_window = self.ui_tree.find_node(node_type="AgentDialogueWindow")
                    btn_group = self.ui_tree.find_node(node_type="ButtonGroup", root=dialog_window)
                    btn_request = btn_group.find_image('images/btn_request_mission.png', 10, 0.7)
                    left_click(btn_request.get_center())
                    time.sleep(2)
                    failsafe(30)
            if decline_count > 10:
                raise Exception("To many declines")
            failsafe(300, msg="Getting mission", timer_index=1)
        right_click(self.ui_tree.find_node({'_name': 'tablecell 0-3'}).get_center())
        time.sleep(1)
        left_click(self.ui_tree.find_node({'_name': 'context_menu_Add Waypoint'}))
    ...
    def can_find_complete_button(self):
        dialog_window = self.ui_tree.find_node(node_type="AgentDialogueWindow")
        btn_group = self.ui_tree.find_node(node_type="ButtonGroup", root=dialog_window)
        btn_complete = btn_group.find_image('images/btn_complete_mission.png', 10, 0.7)
    ...
    def can_accept(self):
        dialog_window = self.ui_tree.find_node(node_type="AgentDialogueWindow")
        btn_group = self.ui_tree.find_node(node_type="ButtonGroup", root=dialog_window)
        btn_accept = btn_group.find_image('images/btn_accept.png', 10, 0.7)
        ...
        while True:
            ...
            while self.inSpace and self.waypointCount == 2:
                ...
                while route_length == 99:
                    time.sleep(1)
                    route_length = self.autopilot.get_route_length()
                    failsafe(delta=15, timer_index=2, msg="route length 99")
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            img.save(fr"FatalError_{utils.fatalErrorCount}.png")
