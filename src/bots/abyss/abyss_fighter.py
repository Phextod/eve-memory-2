...
class AbyssFighter:
    def __init__(self, eve_ui: EveUI):
        ...
        self.main_layer = self.eve_ui.ui_tree.find_node({'_name': 'l_main'})
    ...
    def get_weather_modifiers(self):
        ...
        while not tooltip_panel:
            weather_btn = wait_for_truthy(
                lambda: next((b for b in self.eve_ui.ship_ui.update().buff_buttons if "weather" in b.attrs["_name"]), None),
                10
            )
            move_cursor(weather_btn.get_center())
            time.sleep(1)
            tooltip_panel = wait_for_truthy(lambda: self.eve_ui.ui_tree.find_node(node_type="TooltipPanel", refresh=True), 5)

        while True:
            try:
                percentage_container_1 = self.eve_ui.ui_tree.find_node({"_name": "Row1_Col0"}, root=tooltip_panel)
                penalty_text = self.eve_ui.ui_tree.find_node(
                    node_type="EveLabelMedium",
                    root=percentage_container_1
                ).attrs["_setText"]
                penalty_multiplier = 1.0 + float(penalty_text.split(" ")[0]) / 100

                percentage_container_2 = self.eve_ui.ui_tree.find_node({"_name": "Row2_Col0"}, root=tooltip_panel)
                bonus_text = self.eve_ui.ui_tree.find_node(
                    node_type="EveLabelMedium",
    ...
    def enemy_entries_on_overview(self):
        enemy_entries = []
        for entry in self.eve_ui.overview.entries:
    ...
    def precompute_enemy_ship_attributes(self, enemies):
        self.player: PlayerShip = copy.deepcopy(config.ABYSSAL_PLAYER_SHIP)
        if [e for e in self.eve_ui.overview.update().entries if "Suppressor" in e.type]:
        ...
        for i in range(len(clear_order) - 1, -1, -1):
            ...
            if enemy_amounts_on_overview == enemy_amounts_required:
                if active_stage.orbit_target is not None and next(
                    (
                        e for e in self.eve_ui.overview.entries
    ...
    def manage_navigation(self, clear_order):
        self.eve_ui.ship_ui.update_alert()
        cache_distance = next(e for e in self.eve_ui.overview.entries if "Cache" in e.type).distance_in_meters()
        if cache_distance > 55_000:
            if "Approaching" not in self.eve_ui.ship_ui.indication_text:
                self.eve_ui.overview.lock_order()
                self.eve_ui.overview.update()
                cache_entry = next(e for e in self.eve_ui.overview.entries if "Cache" in e.type)
                self.eve_ui.overview.unlock_order()
                click(cache_entry.node, button=MOUSE_RIGHT)
                self.eve_ui.context_menu.click_safe("Approach")
            return

        if "Orbiting" in self.eve_ui.ship_ui.indication_text or not self.eve_ui.target_bar.targets:
            return
        ...
        overview_min_tag_entry = min(self.eve_ui.overview.entries, key=lambda x: int(x.tag) if x.tag else 10)
        ...
        if current_stage.orbit_target is None:
            orbit_target_entry = next(e for e in self.eve_ui.overview.entries if "Cache" in e.type)
            self.eve_ui.overview.unlock_order()
            click(orbit_target_entry.node, button=MOUSE_RIGHT)
            self.eve_ui.context_menu.open_submenu("Orbit", contains=True)
            self.eve_ui.context_menu.click_safe(DistancePresets.closest(1_000)["text"])
            return
        ...
        if overview_min_tag_entry.distance_in_meters() <= current_stage.orbit_target.optimal_orbit_range * 1.1:
            self.eve_ui.context_menu.open_submenu("Orbit", contains=True)
            self.eve_ui.context_menu.click_safe(
        ...
        while not (track_btn or look_at_my_ship_btn):
            track_btn = self.eve_ui.context_menu.get_menu_btn("Track")
            look_at_my_ship_btn = self.eve_ui.context_menu.get_menu_btn("Look At My Ship")

        if track_btn:
            self.eve_ui.context_menu.click_safe("Track")
    ...
    def select_current_target(self, current_stage):
        self.eve_ui.overview.update()
        self.eve_ui.target_bar.update()

        if current_stage is None:
            return None

        if len(self.eve_ui.target_bar.targets) < 1:
            return False

        if current_stage.target == current_stage.orbit_target:
            min_tag_target = min(
                self.eve_ui.target_bar.targets,
            ...
            target_to_set = next(
                (t for t in self.eve_ui.target_bar.targets if t.tag is None and t.name == current_stage.target),
                None
            )
    ...
    def manage_targeting(self, current_stage: Stage, next_stage: Stage):
        ...
        targeted_entries = [e for e in self.eve_ui.overview.entries if e.is_targeted_by_me]
        if len(targeted_entries) >= len(targets):
            return True
        being_targeted_entry_count = len([e for e in self.eve_ui.overview.entries if e.is_being_targeted])
        ...
        for target in targets:
            matching_entries = [e for e in self.eve_ui.overview.entries if e.type == target[0] and e.tag == target[1]]
    ...
    def manage_weapons(self):
        ...
        self.eve_ui.target_bar.update()
        non_active_targets = [t for t in self.eve_ui.target_bar.targets if not t.is_active_target]
        ...
        weapon_modules = [self.eve_ui.ship_ui.high_modules[i] for i in config.ABYSSAL_WEAPON_MODULE_INDICES]
        ...
        self.eve_ui.ship_ui.update_high_slots()
        ...
        if (
            self.eve_ui.target_bar.get_active_target() is not None
            and self.eve_ui.target_bar.get_active_target().distance <= weapon_range
        ):
            for weapon_module in weapon_modules:
    ...
    def manage_hardeners(self, capacitor_limit, overheat=False):
        should_be_active = self.eve_ui.ship_ui.capacitor_percent > capacitor_limit
        for i in config.ABYSSAL_HARDENER_MODULE_INDICES:
            self.eve_ui.ship_ui.medium_modules[i].set_overload(should_be_active and overheat)
            self.eve_ui.ship_ui.medium_modules[i].set_active(should_be_active)

    def manage_propulsion(self, capacitor_limit):
        for i in config.ABYSSAL_SPEED_MODULE_INDICES:
            self.eve_ui.ship_ui.medium_modules[i].set_active(self.eve_ui.ship_ui.capacitor_percent > capacitor_limit)

    def select_orbit_target(self):
        self.eve_ui.overview.update()
        min_tag_target_entry = min(
            self.eve_ui.overview.entries,
        ...
        self.eve_ui.overview.lock_order()
        self.eve_ui.overview.update()
        orbit_target_entry = next(e for e in self.eve_ui.overview.entries if e.tag == min_tag_target_entry.tag)
        self.eve_ui.overview.unlock_order()
    ...
    def manage_webs(self, capacitor_limit, current_stage: Stage):
        if not current_stage or not self.eve_ui.target_bar.targets:
            return

        web_module_statuses = [
            self.eve_ui.ship_ui.medium_modules[i].active_status == ShipModule.ActiveStatus.active
        ...
        for i in config.ABYSSAL_WEB_MODULE_INDICES:
            if config.ABYSSAL_WEB_RANGE < self.eve_ui.target_bar.targets[0].distance:
                continue

            if self.eve_ui.ship_ui.capacitor_percent < capacitor_limit:
                self.eve_ui.ship_ui.medium_modules[i].set_active(False)
                continue

            if current_stage.orbit_target:
                if current_stage.target != current_stage.orbit_target:
                    self.select_orbit_target()
                self.eve_ui.ship_ui.medium_modules[i].set_active(True)
                if current_stage.target != current_stage.orbit_target:
                    self.select_current_target(current_stage)

    def manage_shield(self, capacitor_limit, overheat=False):
        missing_shield_hp = (1 - self.eve_ui.ship_ui.shield_percent) * config.ABYSSAL_PLAYER_SHIP.shield_max_hp
        should_be_active = not (
            missing_shield_hp < config.ABYSSAL_SHIELD_BOOSTER_AMOUNT
            or self.eve_ui.ship_ui.capacitor_percent < capacitor_limit
        )
    ...
    def manage_modules(self, clear_order, targeting_successful):
        ...
        if not targeting_successful:
            ...
            medium_slot_max_heat_damage = max(m.heat_damage for i, m in self.eve_ui.ship_ui.medium_modules.items())
            overheat_defenses = medium_slot_max_heat_damage < 0.7 and (
                (config.ABYSSAL_IS_SHIELD_TANK and self.eve_ui.ship_ui.shield_percent < 0.65)
                or (config.ABYSSAL_IS_ARMOR_TANK and self.eve_ui.ship_ui.armor_percent < 0.65)
                or (config.ABYSSAL_IS_STRUCTURE_TANK and self.eve_ui.ship_ui.structure_percent < 0.65)
            )
            current_stage, _ = self.get_current_and_next_stage(clear_order)
    ...
    def manage_drones(self):
        ...
        drone_danger = (
            ...
            or [e for e in self.eve_ui.overview.entries if e.is_only_targeting]
        )
        if drone_danger:
        ...
        drones_to_launch = min(
            len(self.eve_ui.drones.in_bay),
            config.ABYSSAL_MAX_DRONES_IN_SPACE - len(self.eve_ui.drones.in_space)
        )
        if drones_to_launch:
            drones_with_shield = [d for d in self.eve_ui.drones.in_bay if d.shield_percent > 0.9]
            self.eve_ui.drones.launch_drones(drones_with_shield[:drones_to_launch])

        drone_recalled = False
        for drone in self.eve_ui.drones.in_space:
            if drone.shield_percent > 0.8:
                continue
            self.eve_ui.drones.recall(drone)
            drone_recalled = True

        self.eve_ui.target_bar.update()
        if (
            not drone_recalled
            and not any(d.status == DroneStatus.returning for d in self.eve_ui.drones.in_space)
            and self.eve_ui.target_bar.get_active_target() is not None
            and self.eve_ui.target_bar.get_active_target().distance < config.ABYSSAL_PLAYER_SHIP.drone_range
        ):
            self.eve_ui.drones.attack_target()
    ...
    def set_orbit_tags(self, clear_order: List[Stage]):
        ...
        for i, target in enumerate(orbit_targets):
            entry_with_tag = next((e for e in self.eve_ui.overview.entries if e.tag == str(i)), None)
            if entry_with_tag:
                if entry_with_tag.type == target.name:
                    continue
                else:
                    entry_with_tag.clear_tag()
            target_entry = next(e for e in self.eve_ui.overview.entries if target.name == e.type and e.tag is None)
            target_entry.set_tag(str(i))

        wait_for_truthy(
            lambda: max(
                [int(e.tag) if e.tag else -1 for e in self.eve_ui.overview.update().entries]
            ) == len(orbit_targets) - 1,
            10
        )
    ...
    def deactivate_modules(self):
        self.eve_ui.ship_ui.update_modules()
        for i, m in self.eve_ui.ship_ui.high_modules.items():
            if i in config.ABYSSAL_WEAPON_MODULE_INDICES:
                continue
            m.set_overload(False)
            m.set_active(False)
        for _, m in self.eve_ui.ship_ui.medium_modules.items():
            m.set_overload(False)
            m.set_active(False)
        for _, m in self.eve_ui.ship_ui.low_modules.items():
            m.set_overload(False)
            m.set_active(False)
    ...
    def open_bio_cache(self):
        ...
        wait_for_truthy(lambda: self.eve_ui.target_bar.update().targets, 20)
        ...
        while len(potential_caches) == 1 and "largeCollidableStructure" in potential_caches[0].icon:
            ...
            if self.eve_ui.target_bar.targets and self.eve_ui.target_bar.targets[0].distance < weapon_max_range / 2:
                self.manage_weapons()
